...
export function FinishedGoodsModule({ onBack }: { onBack: () => void }) {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    let query = supabase.from('cnc_work_orders').select('*');
    if (true) {
      query = supabase.from('cnc_work_orders').select('*').or('status.eq.Completed,completed.gt.0');
    }
  ...
  const filteredRecords = records.filter(r => 
    (r.wo_no || '').toLowerCase().includes(search.toLowerCase()) || 
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full min-h-[500px]">
      <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800">Finished Goods</h2>
            <p className="text-xs text-slate-500">Manage completed production goods</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-200">
                ...
                <th className="p-3 font-semibold">Completed Qty</th>
            ...
            <tbody>
              {filteredRecords.map(record => (
                <tr key={record.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="p-3 text-sm font-mono font-medium text-slate-800">{record.wo_no || (record.party_name && 'wo_no' === 'customer_name' ? record.party_name : '-')}</td>
                  ...
                  <td className="p-3 text-sm text-slate-700">{record.completed || (record.party_name && 'completed' === 'customer_name' ? record.party_name : '-')}</td>
      ...
      <Modal open={!!selectedRecord} onClose={() => setSelectedRecord(null)} title={'Finished Goods Details: ' + (selectedRecord?.wo_no || 'Pending')} size="3xl" footer={
        <div className="flex justify-between w-full">
          <div>
             <Button onClick={() => alert('Development Note: Conversion logic will go here.')} className="bg-emerald-600 hover:bg-emerald-700 text-white">Create DC</Button>
          </div>
          ...
          <div className="flex gap-6">
            <div className="flex-1 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Work Order"><input className={inputClass} value={selectedRecord.wo_no || selectedRecord.party_name || ''} disabled /></FormField>
                ...
                <FormField label="Completed Qty"><input className={inputClass} value={selectedRecord.completed || selectedRecord.party_name || ''} disabled /></FormField>
            ...
            <div className="w-64 bg-slate-50 border border-slate-200 rounded-lg p-4 flex flex-col h-full">
              ...
              <div className="flex flex-col space-y-0 relative pl-4">
                ...
                <FlowStep active={'SALES ORDER' === 'FINISHED GOODS'} highlight={'SALES ORDER' === 'FINISHED GOODS'} title="SALES ORDER" subtitle={'SALES ORDER' === 'FINISHED GOODS' ? selectedRecord.wo_no : (selectedRecord.sales_order_no || 'Linked')} />
                <FlowStep active={'INWARD' === 'FINISHED GOODS'} highlight={'INWARD' === 'FINISHED GOODS'} title="INWARD" subtitle={'INWARD' === 'FINISHED GOODS' ? selectedRecord.wo_no : 'Linked'} />
                <FlowStep active={'PRODUCTION' === 'FINISHED GOODS'} highlight={'PRODUCTION' === 'FINISHED GOODS'} title="PRODUCTION" subtitle={'PRODUCTION' === 'FINISHED GOODS' ? selectedRecord.wo_no : 'Linked'} />
                <FlowStep active={'FINISHED GOODS' === 'FINISHED GOODS'} highlight={'FINISHED GOODS' === 'FINISHED GOODS'} title="FINISHED GOODS" subtitle={'FINISHED GOODS' === 'FINISHED GOODS' ? selectedRecord.wo_no : 'Linked'} />
                <FlowStep active={'DELIVERY CHALLAN' === 'FINISHED GOODS'} highlight={'DELIVERY CHALLAN' === 'FINISHED GOODS'} title="DELIVERY CHALLAN" subtitle={'DELIVERY CHALLAN' === 'FINISHED GOODS' ? selectedRecord.wo_no : 'Linked'} />
                <FlowStep active={'INVOICE' === 'FINISHED GOODS'} highlight={'INVOICE' === 'FINISHED GOODS'} title="INVOICE" subtitle={'INVOICE' === 'FINISHED GOODS' ? selectedRecord.wo_no : 'Linked'} isLast />
              </div>
            </div>
          </div>
        )}

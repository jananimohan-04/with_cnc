...
export function InwardModule({ onBack }: { onBack: () => void }) {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    let query = supabase.from('cnc_inwards').select('*');
    if (false) {
      query = supabase.from('cnc_inwards').select('*');
    }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full min-h-[500px]">
            ...
            <tbody>
              {filteredRecords.map(record => (
                <tr key={record.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="p-3 text-sm font-mono font-medium text-slate-800">{record.inward_no || (record.party_name && 'inward_no' === 'customer_name' ? record.party_name : '-')}</td>
                  <td className="p-3 text-sm font-semibold text-brand-700">{record.party_name || (record.party_name && 'party_name' === 'customer_name' ? record.party_name : '-')}</td>
      ...
      <Modal open={!!selectedRecord} onClose={() => setSelectedRecord(null)} title={'Inwards Details: ' + (selectedRecord?.inward_no || 'Pending')} size="3xl" footer={
          ...
          <div className="flex gap-6">
            <div className="flex-1 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Inward No"><input className={inputClass} value={selectedRecord.inward_no || selectedRecord.party_name || ''} disabled /></FormField>
                <FormField label="Party"><input className={inputClass} value={selectedRecord.party_name || selectedRecord.party_name || ''} disabled /></FormField>
            ...
            <div className="w-64 bg-slate-50 border border-slate-200 rounded-lg p-4 flex flex-col h-full">
              ...
              <div className="flex flex-col space-y-0 relative pl-4">
                ...
                <FlowStep active={'SALES ORDER' === 'INWARD'} highlight={'SALES ORDER' === 'INWARD'} title="SALES ORDER" subtitle={'SALES ORDER' === 'INWARD' ? selectedRecord.inward_no : (selectedRecord.sales_order_no || 'Linked')} />
                <FlowStep active={'INWARD' === 'INWARD'} highlight={'INWARD' === 'INWARD'} title="INWARD" subtitle={'INWARD' === 'INWARD' ? selectedRecord.inward_no : 'Linked'} />
                <FlowStep active={'PRODUCTION' === 'INWARD'} highlight={'PRODUCTION' === 'INWARD'} title="PRODUCTION" subtitle={'PRODUCTION' === 'INWARD' ? selectedRecord.inward_no : 'Linked'} />
                <FlowStep active={'FINISHED GOODS' === 'INWARD'} highlight={'FINISHED GOODS' === 'INWARD'} title="FINISHED GOODS" subtitle={'FINISHED GOODS' === 'INWARD' ? selectedRecord.inward_no : 'Linked'} />
                <FlowStep active={'DELIVERY CHALLAN' === 'INWARD'} highlight={'DELIVERY CHALLAN' === 'INWARD'} title="DELIVERY CHALLAN" subtitle={'DELIVERY CHALLAN' === 'INWARD' ? selectedRecord.inward_no : 'Linked'} />
                <FlowStep active={'INVOICE' === 'INWARD'} highlight={'INVOICE' === 'INWARD'} title="INVOICE" subtitle={'INVOICE' === 'INWARD' ? selectedRecord.inward_no : 'Linked'} isLast />
              </div>
            </div>
          </div>
        )}
